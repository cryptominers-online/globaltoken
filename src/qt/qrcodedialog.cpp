...
{
    QString ret = QString("globaltoken:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non GTC unit input in lnReqAmount, we generate the URI with GTC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::GTC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
