...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "globaltoken.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No globaltoken.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override globaltoken.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "globaltokend.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Globaltoken will not work properly.");
